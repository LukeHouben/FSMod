//UGX Notes: Changed the code so that you can set any price you want on a blocker without adding a new string to zombie.str. Also fixed some bugs with the swinging door section to make things more clear.
// Also added a HUD which displays blocker errors when developer 1 is set in console.
...
door_think()
{
	...
	while( 1 )
	{
		...
		else
		{
			...
			if( is_player_valid( who ) && isDefined(who))
			{
		...
		for(i=0;i<self.doors.size;i++)
		{
			...
			if(i == 0) //UGX
			{
				if ( IsDefined( self.script_sound ))
					if(self.script_sound != "none") //UGX
						playsoundatposition( self.script_sound, self.doors[i].origin );
				else
					playsoundatposition( "door_slide_open", self.doors[i].origin );
			}
			...
			if(!isDefined(self.doors[i].script_string)) self.doors[i].script_string = "none";

			switch( self.doors[i].script_string )
			{
			case "rotate":
				if(!modderHelp(self.doors[i].script_angles, "No script_angles set on door(s). The door will not rotate until this is fixed!"))
				{
			...
			case "move":
			case "slide_apart":
				if(!modderHelp(self.doors[i].script_vector, "No script_vector KVP set on door(s). The door will not move until this is fixed!"))
				{
...
//						self.doors[i] animscripted( "door_anim", self.doors[i].origin, self.doors[i].angles, level.scr_anim[ self.doors[i].script_animname ] );
			...
			default: 
				modderHelp((isDefined(self.doors[i].script_string)), "No script_string specified on door. The door entity does not know how to move until this is fixed!", "test");
				break;
			}
		...
		//UGX_SCRIPT BEGIN Needed for hacker device
...
blocker_trigger_think()
{
	...
	while( 1 )
	{
		...
		while( 1 )
		{
			...
			wait 0.75;
			earthquake( 0.5, 0.25, self.origin, 512 );
			wait 0.25;
